// ⚠️ DESTRUCTIVE — this script DROPS every users/predictions/notifications/
// uploads/messages/settings document and inserts demo data. It is for
// fresh-install / demo seeding ONLY. To prevent accidental data loss it now
// requires the explicit `--wipe` flag.
//
//   safe (no-op):           node lib/seed.js
//   destructive (data loss): node lib/seed.js --wipe
...
if (!WIPE) {
  console.log("\n⛔  Refusing to run.\n");
  console.log("    lib/seed.js DELETES every user, prediction, notification,");
  console.log("    upload, message, and settings document, then inserts demo data.");
  console.log("    To actually run it, re-invoke with --wipe:\n");
  console.log("      node lib/seed.js --wipe\n");
  console.log("    If you only want to update payment addresses (BTC/USDT/MoMo)");
  console.log("    on an EXISTING database without wiping anything, use:\n");
  console.log("      node scripts/update-payments.js\n");
  process.exit(0);
}
