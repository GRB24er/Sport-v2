...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    setError("");
    if (!login || !password) return setError("Phone number and password are required.");
    if (login.length < 3) return setError("Enter a valid phone number.");
    setLoading(true);
    try {
    ...
    <div style={{minHeight:"100vh",background:"#0B0D10",color:"#F0F0F2",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div className="lcontent" style={{position:"relative",zIndex:1,flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 20px"}}>
        <div className="lcard" style={{width:"100%",maxWidth:420,background:"#12141A",border:"1px solid #1E2028",borderRadius:20,padding:"40px 36px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",marginBottom:32}}><a href="/"><Logo height={70} /></a></div>
          ...
          <form onSubmit={handleLogin}>
            <div style={{marginBottom:18}}><label style={{display:"block",fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:"#555",marginBottom:6}}>Phone Number</label><input type="text" placeholder="Enter your phone number" value={login} onChange={e=>setLogin(e.target.value)} style={{width:"100%",padding:"14px 16px",background:"#0B0D10",border:"1px solid #1E2028",borderRadius:10,color:"#F0F0F2",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}} /></div>
            ...
            {error && <div style={{background:"#D4AF3710",border:"1px solid #D4AF3725",borderRadius:10,padding:"12px 16px",marginBottom:18,fontSize:13,color:"#D4AF37",fontWeight:600}}>{"⚠ "+error}</div>}
          ...
          <div style={{textAlign:"center",marginTop:24,fontSize:13,color:"#555"}}>{"Don't have an account? "}<a href="/signup" style={{color:"#D4AF37",fontWeight:700,textDecoration:"none"}}>Sign Up</a></div>
        </div>
      </div>
      <div style={{position:"relative",zIndex:1,textAlign:"center",padding:20,fontSize:11,color:"#333",borderTop:"1px solid #1E2028"}}><Logo height={28} style={{opacity:0.4,marginBottom:8}} /><div>{"© 2026 BetGenius AI. 18+ Only."}</div></div>
    </div>
  );
}
